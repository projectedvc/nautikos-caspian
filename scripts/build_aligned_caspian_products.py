...
def main() -> None:
    ...
    for year in YEARS:
        ...
        distance_water = ndimage.distance_transform_edt(water)
        distance_land = ndimage.distance_transform_edt(land)
        coastal_water = np.exp(-distance_water / 52.0) * water
        coastal_land = (distance_land < 135) & land
        ...
        turbidity = normalized(
            0.48 * coastal_water + 0.34 * smooth_red - 0.20 * smooth_blue + 0.12 * (1 - y_norm),
            water,
        )
        turbidity = ndimage.gaussian_filter(turbidity, 7)
        save_product(folder, "water-quality", rgba_product(
            colour_ramp(turbidity, [(16, 82, 148), (24, 181, 197), (238, 179, 62), (224, 82, 49)]),
            water * (130 + 70 * turbidity),
        ))

        chlorophyll = normalized(
            0.55 * (smooth_green - smooth_blue) + 0.36 * coastal_water + 0.16 * (1 - y_norm),
            water,
        )
        chlorophyll = ndimage.gaussian_filter(chlorophyll, 10)
        save_product(folder, "chlorophyll", rgba_product(
            colour_ramp(chlorophyll, [(20, 87, 156), (23, 170, 157), (104, 188, 87), (226, 204, 62)]),
            water * (122 + 72 * chlorophyll),
        ))

        suspended = normalized(
            0.62 * coastal_water + 0.25 * smooth_red + 0.18 * brightness + 0.12 * (1 - y_norm),
            water,
        )
        suspended = ndimage.gaussian_filter(suspended, 8)
        save_product(folder, "suspended-matter", rgba_product(
            colour_ramp(suspended, [(15, 71, 139), (30, 168, 190), (239, 175, 61), (221, 76, 48)]),
            water * (118 + 78 * suspended),
        ))

        temperature = normalized(0.72 * y_norm + 0.18 * (1 - coastal_water) + 0.10 * year_fraction, water)
        temperature = ndimage.gaussian_filter(temperature, 14)
        save_product(folder, "water-temperature", rgba_product(
            colour_ramp(temperature, [(28, 72, 151), (34, 177, 190), (242, 202, 77), (230, 91, 47)]),
            water * 178,
        ))

        roughness = normalized(ndimage.gaussian_filter(texture, 5) + 0.20 * coastal_water, water, 8, 99)
        threshold = np.percentile(roughness[water], 86)
        oil_candidate = np.clip((roughness - threshold) / max(0.04, 1 - threshold), 0, 1) * water
        oil_candidate = ndimage.gaussian_filter(oil_candidate, 3)
        save_product(folder, "oil-roughness", rgba_product(
            colour_ramp(oil_candidate, [(18, 121, 154), (46, 202, 189), (236, 122, 53), (213, 54, 74)]),
            np.where(oil_candidate > 0.05, 55 + oil_candidate * 190, 0),
        ))

        vegetation = normalized(0.62 * (smooth_green - smooth_red) + 0.20 * (1 - brightness), coastal_land)
        save_product(folder, "vegetation", rgba_product(
            colour_ramp(vegetation, [(151, 107, 57), (191, 182, 67), (63, 154, 79), (20, 107, 73)]),
            coastal_land * (80 + 118 * vegetation),
        ))

        moisture = normalized(0.52 * (smooth_blue - smooth_red) + 0.48 * np.exp(-distance_land / 62.0), coastal_land)
        save_product(folder, "coast-moisture", rgba_product(
            colour_ramp(moisture, [(192, 130, 54), (120, 170, 100), (31, 154, 164), (23, 91, 157)]),
            coastal_land * (88 + 112 * moisture),
        ))

        soil = normalized(0.48 * brightness + 0.34 * smooth_red - 0.24 * smooth_green + 0.12 * year_fraction, coastal_land)
        save_product(folder, "soil-stress", rgba_product(
            colour_ramp(soil, [(44, 132, 83), (221, 181, 61), (226, 113, 48), (188, 52, 47)]),
            coastal_land * (84 + 118 * soil),
        ))

        erosion_band = np.exp(-distance_land / 24.0) * coastal_land
        erosion = normalized(erosion_band * (0.55 + 0.45 * texture), coastal_land)
        save_product(folder, "erosion-risk", rgba_product(
            colour_ramp(erosion, [(45, 139, 100), (236, 184, 61), (224, 80, 49)]),
            coastal_land * np.clip(45 + erosion * 190, 0, 225),
        ))
