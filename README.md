...
В репозитории уже зафиксированы отдельные каталоги Sentinel-2 L2A для каждого года 2020–2026. Для каждого MGRS-сектора выбраны три наименее облачные июльские сцены; каталог года содержит 444–447 реальных сцен и уникальный `scene_set_id`. Несколько наблюдений одного сектора позволяют маскировать облака и не закрывать их однотонными прямоугольниками. Переключение года действительно меняет набор сцен, а масштабирование использует тот же год и тот же продукт без подмены изображения другой картой.
...
Загрузчик поддерживает продолжение `.part`-файлов и дедупликацию URL. Он сохраняет только выбранные сцены вокруг Каспия и необходимые каналы `B02`, `B03`, `B04`, `B08`, `SCL`; готовый `visual/TCI` намеренно исключён. Глобальный архив Земли не скачивается. Полный сырой набор из 3124 сцен занимает ориентировочно 1,1–1,8 ТБ в зависимости от фактического сжатия COG. После построения итоговых COG/тайлов исходные дубли наблюдений можно архивировать или удалить. Обзорный и аналитический кэш уровней 3–9 обычно занимает 10–40 ГБ.
